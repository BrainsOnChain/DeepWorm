...
int16_t ctm_get_current_state(Connectome *const c, const uint16_t id) {
...
static void ctm_set_next_state(Connectome *const c, const uint16_t id,
                               const int16_t val) {
  if (id < c->_neurons_tot) {
    if (val > 127) {
...
static void ctm_add_to_next_state(Connectome *const c, const uint16_t id,
                                  const int8_t val) {
...
static void ctm_iterate_state(Connectome *const c) {
  memcpy(c->_neuron_current, c->_neuron_next,
         c->_neurons_tot * sizeof(c->_neuron_next[0]));
  memcpy(c->_muscle_current, c->_muscle_next,
         c->_muscles_tot * sizeof(c->_muscle_next[0]));
...
static void ctm_meta_flag_discharge(Connectome *const c, const uint16_t id,
                                    const uint8_t val) {
  if (val == 0) {
...
void ctm_init(Connectome *const c) {
  ...
  memset(c->_neuron_current, 0,
         c->_neurons_tot * sizeof(c->_neuron_current[0]));
  memset(c->_neuron_next, 0, c->_neurons_tot * sizeof(c->_neuron_next[0]));
  memset(c->_muscle_current, 0,
...
void ctm_ping_neuron(Connectome *const c, const uint16_t id) {
  ...
  const uint16_t len =
      READ_WORD(NEURAL_ROM, id + 2) - READ_WORD(NEURAL_ROM, id + 1);

  for (int8_t i = 0; i < len; i++) {
    NeuronConnection neuron_conn =
        parse_rom_word(READ_WORD(NEURAL_ROM, address + i));
...
void ctm_neural_cycle(Connectome *const c, const uint16_t *stim_neuron,
                      const uint16_t len) {
...
void ctm_weight_query(Connectome *const c, const uint16_t *input_id,
                      uint16_t *query_result, const uint16_t len_query) {
...
void ctm_discharge_query(Connectome *const c, const uint16_t *input_id,
                         uint8_t *query_result, const uint16_t len_query) {
  for (uint16_t i = 0; i < len_query; i++) {
    uint16_t id = input_id[i];
    uint8_t discharged = c->_meta[id] >> 7;
